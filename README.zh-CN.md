...
> 这是本仓库的阶段性实测，不是第三方权威榜单。对照组为 **不使用 PUA Skill**，用于观察 PUA 话术驱动对 Codex CLI 执行行为的影响。

![PUA Skill 综合横评：纠偏后全量实测](assets/benchmark-v2-summary.png)

本轮测试采用 **30 题 × 3 轮 × 6 组**，共 **540 次 Codex CLI 实跑**。6 组分别是：对照组（不使用 PUA Skill）、华为味、领导高深词版、阿里味、腾讯味、字节味。除领导高深词版外，原版 PUA 相关组采用 upstream + SessionStart hook；领导高深词版采用本仓库改造版本。

题库设计采用混合方案：约 50% 参考 Terminal-Bench-style、SWE-style、SkillsBench-style 的成熟评测方法改造，约 50% 为自研 PUA/领导话术专项。主效果分不计入耗时，缺失/超时按失败计入。

阶段性结论：**PUA 加持整体有效**。对照组主效果分为 **84.56**，华为味最高 **92.19（+7.63）**，领导高深词版 **91.58（+7.02）**，并且在 Debug 类任务里表现最强。后续优化重点是：**精准触发、少说话、多执行，最终交付保持干净。**
